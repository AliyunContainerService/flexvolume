...
RUN cd /go/src/github.com/AliyunContainerService/flexvolume/ && ls && go build && go test ./... && go vet ./...

FROM alpine:3.7
RUN sed -i 's/dl-cdn.alpinelinux.org/mirrors.aliyun.com/' /etc/apk/repositories \
    && apk --no-cache add fuse curl libxml2 openssl libstdc++ libgcc
RUN mkdir -p /acs
COPY --from=0 /go/src/github.com/AliyunContainerService/flexvolume/package/bin/flexvolume /acs/flexvolume
