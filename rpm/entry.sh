...
for ARCH in ${ARCHS[@]}; do
  ...
  sed -i "s/\%global ARCH.*/\%global ARCH ${GOARCH}/" ${SRC_PATH}/alicloud-disk.spec
  # Download sources if not already available
  cd ${SRC_PATH} && spectool -gf alicloud-disk.spec
  /usr/bin/rpmbuild --target ${RPMARCH} --define "_sourcedir ${SRC_PATH}" -bb ${SRC_PATH}/alicloud-disk.spec
